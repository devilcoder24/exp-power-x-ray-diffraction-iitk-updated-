...
voltageSlider.addEventListener("input", function () {
  ...
  // Enable confirm button once user changes voltage
  document.getElementById("confirm-voltage").disabled = false;
});

currentSlider.addEventListener("input", function () {
  currentValue.textContent = `${currentSlider.value} mA`;

   // Enable confirm current button when slider moves
...
function confirmVoltage() {
    let voltage = document.getElementById("voltage-slider").value;

    // Lock voltage slider
    document.getElementById("voltage-slider").disabled = true;
    document.getElementById("confirm-voltage").disabled = true;

    // Enable Current slider after voltage is confirmed
    document.getElementById("current-slider").disabled = false;

    typeWriter("Voltage set to " + voltage + " kV. Now adjust and set the Current.", 30);

    speakInstruction("Voltage has been confirmed. Now set the current.");
}


function confirmCurrent() {
    let current = document.getElementById("current-slider").value;

    // Disable slider and button
    document.getElementById("current-slider").disabled = true;
    document.getElementById("confirm-current").disabled = true;

    // Enable Start Angle input
    document.getElementById("start-angle").disabled = false;

    // Instructions
    typeWriter("Current set to " + current + " mA. Now enter the Start Angle (2θ).", 30);
    speakInstruction("Current has been confirmed. Now enter the start angle for scanning.");
}


// ==========================
// SCAN TIME CALCULATION FUNCTION
// ==========================

function calculateScanTime() {
    // Get values from inputs
    let startAngle = parseFloat(document.getElementById("start-angle").value);
    let endAngle = parseFloat(document.getElementById("end-angle").value);
    let stepSize = parseFloat(document.getElementById("options1").value); // Step Size in ° (optional)
    let scanRate = parseFloat(document.getElementById("options2").value); // Scan Rate in °/min

    // Check if values are valid
    if (isNaN(startAngle) || isNaN(endAngle) || isNaN(scanRate) || startAngle >= endAngle || scanRate <= 0) {
    ...
    // Scan time calculation (minutes)
    // Formula: ScanTime = TotalAngle / ScanRate
    let scanTime = totalAngle / scanRate;

    // Optional: you can consider step size for number of steps (not necessary for time)
    // let numberOfSteps = Math.ceil(totalAngle / stepSize);

    // Update the scan time output
    document.getElementById("result").value = scanTime.toFixed(2) + " minutes";
}

// ==========================
// Event listeners to auto-calculate
// ==========================
document.getElementById("start-angle").addEventListener("input", calculateScanTime);
document.getElementById("end-angle").addEventListener("input", calculateScanTime);
document.getElementById("options1").addEventListener("input", calculateScanTime); // Step size change
document.getElementById("options2").addEventListener("input", calculateScanTime); // Scan rate change
